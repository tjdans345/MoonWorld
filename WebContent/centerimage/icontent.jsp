...
<%@page import="icomment.iCommentBean"%>
...
<html>
	...
	function fnDel(num, name) {
		...
		if(result == true) {
			...
			location.href="deleteicontent.jsp?num=" + num +"&name=" + name;
		} 
	}
	<%-- 댓글삭제 한번 더  물어보는 함수 --%>
	function fnCoDel(conum, num, pageNum) {
		...
		if(result == true) {
			...
			location.href="icodelcomment.jsp?conum="+conum+"&num="+num+"&pageNum="+pageNum;
		} 
	}
	...
	<%
		...
		String DBFilename = boardBean.getFilename();
		String DBFilerealname =  boardBean.getFilerealname();
		//글내용이 존재 한다면 //내용 엔터 처리
		...
		<div id="page-wrapper">
			...
			<!-- Main -->
				<div id="main" class="wrapper style1">
							...
							<section>
								...
								<div class="table-wrapper">
									<table class="alt">
										...
										<tr>
											<td>업로드 된 파일</td>
											<td colspan="3"><%=DBFilerealname%></td>
										</tr>
										<tr>
											<td>첨부된 이미지</td>
											<td colspan="3"><img src="../image/<%=DBFilerealname%>" width="800px" height="750px"></td>
										</tr>
										<tr>
											<td>글내용</td>
					...
					<div class="12u$" align="center">
							...
							<div id="content">
							<input type="button" value="글수정" class="special" onclick="location.href='iupdate.jsp?pageNum=<%=pageNum%>&num=<%=DBnum%>'" />
							<input type="button" value="글삭제" onclick = "fnDel(<%=DBnum%>, '<%=DBId%>')" />
								...
								<%
									...
									//순서1 추가 끝
								%>
								<input type="button" value="목록보기" onclick = "location.href='inotice.jsp?pageNum=<%=pageNum%>'" />
								</div>
					</div>
					<br/>
				...
				%>
					...
					<div class="container">
					<form action="icomment.jsp?num=<%=num%>&pageNum=<%=pageNum%>" method="post" name="cm">
			...
			%>
									...
									<%
										if(count > 0 ) {
											for(int i = 0; i<list.size(); i++) {
												iCommentBean bean = list.get(i);
									%>			
										<tr>
											<th><%=bean.getId()%> &nbsp;&nbsp;|&nbsp; 06.04.23:25&nbsp;&nbsp;&nbsp;<br/><br/><%=bean.getContent()%></th>
											<%
											for(int j=0; j<=10; j++) {
											%>	
											  <td>&nbsp;&nbsp;&nbsp; </td>
					...
					if(count>0) {
						...
						%>
							<a href="icontent.jsp?num=<%=num%>&pageNum=<%=startPage-pageBlock%>">[이전]</a>	
						<%
						}
						
						for(int i=startPage; i<=endPage; i++){
						%>
							<a href="icontent.jsp?num=<%=num%>&pageNum=<%=i%>">&nbsp;[<%=i%>]&nbsp;&nbsp;</a>
						<%
						}
						%>
						<%
						if(endPage < pageCount) {
						%>
							<a href="icontet.jsp?num=<%=num%>&pageNum=<%=startPage+pageBlock%>">[다음]</a>
						<%	
						}
					}
					%>
