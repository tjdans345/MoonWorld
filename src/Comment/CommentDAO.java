...
public class CommentDAO {
	...
	private void freeResource() {
	 ...
	 //댓글 추가하는 메소드
	 public void insertBoard(CommentBean bBean) {
			...
			try {
				...
				//sql 내장함수 MAX(컬럼명)
				sql = "select max(num) from comment"; //가장 큰 글번호 가져오기
				...
				//3 insert
				sql = "insert into comment" + "(id, content, num, date, pnumber)" + "values(?,?,?,?,?)";
	 ...
	 public int getBoardCount(int DBnum) {
			...
			try {
				...
				sql="select * from comment where pnumber=?";
				pstmt=con.prepareStatement(sql);
	 ...
	 //댓글 뿌려주는 메소드
	 public List<CommentBean> getBoardList(int startRow, int pageSize, int DBnum) {
			...
			try {
				...
				sql = "select * from comment where pnumber like ? order by num desc limit ?,?";
				pstmt=con.prepareStatement(sql);
				pstmt.setInt(1, DBnum);
